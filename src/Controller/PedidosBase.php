...
class PedidosBase extends AbstractController
{
  ...
  public function mostrarProductos($id)
  {
    ...
    $nombre = $this->getDoctrine()->getRepository(Categoria::class)
      ->find($id)->getNombre(); // devuelve el nombre de la categoría
    $codCat = $this->getDoctrine()->getRepository(Categoria::class)
    ->find($id)->getCodCat(); // devuelve el código de la categoría

    if (!$productos) {
      throw $this->createNotFoundException('Categoría no encontrada');
    }
    return $this->render("productos.html.twig", 
      array('productos' => $productos, 'nombre' => $nombre, 'codCat' => $codCat,
             'prodOut' => false));
  }

  /**
   * @Route("/productos/{id}/outstock/{codProd}", name="productosOutStock")
   */
  public function mostrarProductosFueraStock($id, $codProd)
  {
    $productos = $this->getDoctrine()->getRepository(Categoria::class)->find($id)->getProductos();
    $nombre = $this->getDoctrine()->getRepository(Categoria::class)
      ->find($id)->getNombre(); // devuelve el nombre de la categoría
    $codCat = $this->getDoctrine()->getRepository(Categoria::class)
    ->find($id)->getCodCat(); // devuelve el código de la categoría
    $prodOut = $this->getDoctrine()->getRepository(Producto::class)
    ->find($codProd); // devuelve el producto fuera de stock
  ...
  public function realizarPedido(SessionInterface $session, \Swift_Mailer $mailer)
  {
    ...
    $message = (new \Swift_Message())
      ->setFrom('noreply@empresafalsa.com', 'Pedidos')
      //->setTo($this->getUser()->getCorreo())
      ->setTo('antoal898@gmail.com')
  ...
  public function mostrarCarrito(SessionInterface $session)
  {
    ...
    foreach ($carrito as $codigo => $cantidad) {
      ...
      $elem['stock'] = $producto->getStock();
      $elem['descripcion'] = $producto->getDescripcion();
      $elem['urlimg'] = $producto->getUrlImg();
      $elem['unidades'] = implode($cantidad);
      $productos[] = $elem;
    }
  ...
  public function anadir(SessionInterface $session)
  {
    ...
    if (isset($carrito[$id])) {
      $unidades_totales = $unidades + $carrito[$id]['unidades'];
      if ($stock < $unidades_totales) {
        return $this->redirectToRoute("productosOutStock", array('id' => $codCat, 'codProd' => $id)); 
        // redirige a la categoría en la que estaba pasando el producto fuera de stock
      } else {
    ...
    $session->set('carrito', $carrito);
    return $this->redirectToRoute("productos", array('id' => $codCat)); 
    // redirige a la categoría en la que estaba
  }
